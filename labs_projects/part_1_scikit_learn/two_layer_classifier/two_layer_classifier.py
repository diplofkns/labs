...
class TwoLayerClassifier(object):
    ...
    def predict(self, x):
        ...
        if len(x.shape) == 1:  # Predict on one sample
            score = self.net.forward(x) #Fonction de haut niveau : on fait le forward pass en appelant une fonction de la classe DenseLayer
            return np.argmax(score) #most-probable class ie. score le plus élevé avec un softmax

        elif len(x.shape) == 2:  # Predict on multiple samples
            scores = np.array([self.net.forward(sample) for sample in x]) #Même chose mais avec une boucle sur le nombre de données
            return np.argmax(scores, axis=1) #most-probable class ie. score le plus élevé avec un softmax
    ...
    def global_accuracy_and_cross_entropy_loss(self, x, y, l2_r=-1.0):
        ...
        for i, sample in enumerate(x):
            scores = self.net.forward(sample) #Sortie de forward : vecteur de score correpsondant à des probabilités d'apartenance pour chaque classe
            loss_i, _ = self.net.cross_entropy_loss(scores, y[i]) 
            loss += loss_i
            
            if np.argmax(scores) == y[i]: #condition : donnée bien classée si l'indice de classe de la probabilité la plus élevée = classe vraie de la donnée
                accu += 1

        loss /= len(x) #loss moyenne
        accu /= len(x) #accu moyenne
        return accu, loss
    ...
    def momentum_update(self, w, dw, lr, mu):
        ...
        v = mu * v_prev - lr * dw #m.a.j. de v pour un momentum (sans RMS prop)
        w += v #m.a.j. de w
...
class TwoLayerNet(object):
    ...
    def cross_entropy_loss(self, scores, y):
        ...
        loss = -np.log(probs[y])
        loss += 0.5 * self.l2_reg * np.sum(self.layer1.W ** 2) + 0.5 * self.l2_reg * np.sum(self.layer2.W ** 2)
        
        dloss_dscores = probs
        dloss_dscores[y] -= 1 #sans la regularisation car elle est déjà implémentée
...
class DenseLayer(object):
    ...
    def forward(self, x):
        ...
        x = augment(x)
        a = np.dot(self.W.T,x) 
        f = np.maximum(0,a) #Activation ReLU
